...
{
    struct cirListDeque *list = malloc(sizeof(struct cirListDeque));
    struct Vertex *vert = source;
    
    initCirListDeque(list);
    clearVisited(g);
    addFrontCirListDeque(list, vert);
    
    while(!isEmptyCirListDeque(list))
    {
        vert = frontCirListDeque(list);
        ...
        for(int i = 0; i < vert -> numNeighbors; i++)
        {
            if(!vert -> neighbors[i] -> isVisited)
            {
                addFrontCirListDeque(list, vert -> neighbors[i]);
            }
        }
    }
